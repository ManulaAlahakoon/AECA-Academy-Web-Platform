...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    const uploadPath = path.join(baseUploadPath, req.body.course);
...
const fileFilter = (req, file, cb) => {
  console.log("Assignment upload MIME type:", file.mimetype);
  const allowed = [
    "application/pdf",
    "application/msword",
...
export const checkFileReceived = (req, res, next) => {
  console.log("File received in controller:", req.file);
  if (!req.file) {
    return res.status(400).json({ success: false, message: "No file received. Check MIME type." });
  }
  next();
};
