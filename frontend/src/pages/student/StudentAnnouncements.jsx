...
const StudentAnnouncements = () => {
  ...
  const AnnouncementItem = ({ ann }) => (
    <li className="bg-white shadow-md border rounded-lg p-4 relative">
      <div className="flex items-center justify-between">
        <span
          className={`text-xs font-semibold px-2 py-1 rounded-full ${
            ann.isPublic ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-800"
          }`}
        >
          {ann.course}
        </span>
      ...
      <div className="flex items-start mt-3 gap-2">
        <div className="pt-1 text-[#800000]">
          {ann.isPublic ? <FaBullhorn /> : <FaBookOpen />}
        </div>
        <div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold text-center text-[#800000] mb-8">
      ...
      {loading ? (
        ...
        <>
          <section className="mb-10">
            <h2 className="text-xl font-semibold text-[#800000] mb-4 border-b pb-2">
              🔔 Recent Announcements
            </h2>
            {recentPosts.length === 0 ? (
              ...
              <ul className="space-y-4">
                {recentPosts.map((ann) => (
                  <AnnouncementItem key={ann._id} ann={ann} />
                ))}
              </ul>
            )}
          </section>
